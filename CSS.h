void append_page_header() {
  ...
  webpage += F("<head>");
  webpage += F("<link href='https://fonts.googleapis.com/icon?family=Material+Icons' rel='stylesheet'>");
  webpage += F("<link rel='stylesheet' href='https://maxcdn.bootstrapcdn.com/bootstrap/3.3.7/css/bootstrap.min.css'>");
  webpage += F("<script src='https://ajax.googleapis.com/ajax/libs/jquery/3.2.1/jquery.min.js'></script>");
  webpage += F("<script src='https://maxcdn.bootstrapcdn.com/bootstrap/3.3.7/js/bootstrap.min.js'></script>");
  ...
  webpage += F("<style>");
  webpage += F("body{margin:0 auto;font-family:arial;text-align:center;background-color:#F7F2Fd;}");
  webpage += F("ul{list-style-type:none;margin:0;padding:0;overflow:hidden;background-image: linear-gradient(#558ED5, #a0b0ff 40%, #a0b0ff 60%, #558ED5);font-size:1em;}");
  webpage += F("li{float:left;border-radius:0;}last-child {border-right:none;font-size:85%}");
  webpage += F("a {color:white;}");
  webpage += F("li a{display: block;border-radius:0.375em;padding:0.44em 0.44em;text-decoration:none;font-size:85%}");
  webpage += F("li a:hover{background-color:#EAE3EA;border-radius:0;font-size:85%}");
  webpage += F("section {font-size:0.88em;}");
  webpage += F("h1{color:white;font-size:1em;padding:0.2em 0.2em;background:#558ED5;margin:0;}");
  webpage += F("h2{color:orange;font-size:1.0em;}");
  webpage += F("h3{font-size:0.8em;font-weight:900;}");
  webpage += F("table{max-width:600px;font-family:arial,sans-serif;font-size:0.9em;border-collapse:collapse;width:85%;}"); 
  webpage += F("th,td {text-align:left;padding:0.3em;border-bottom:0.06em solid #dddddd;}"); 
  webpage += F(".file-list-header {background-color: black !important;color: white;}");
  ...
  webpage += F(".buttons {border-radius:0.5em;background:#558ED5;padding:0.3em 0.3em;color:white;font-size:80%;}");
  ...
  webpage += F(".file-dir-icon{width:20px;}");
  webpage += F(".delete-icon{color:red;cursor:pointer;}");
  webpage += F(".file-link:hover{cursor:pointer; text-decoration:underline;}");
  webpage += F(".file-upload-block {border: 1px solid grey; border-radius: 10px; max-width: 600px;width: 85%; margin: auto;}"); 
  webpage += F("input {margin: auto;}");
  webpage += F("</style></head><body><h1>File Server "); 
  webpage += String(ServerVersion) + "</h1>";
}
//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
void append_page_menu() {
  webpage += F("<ul>");
...
void append_page_footer(){ // Saves repeating many lines of code for HTML page footers
  ...
  webpage += F("</body></html>");
  webpage += F("<script>\nfunction downloadFile(obj){\n $('input[name =\"download\"]').val(obj.textContent); \n $('#form-download').submit(); \n }\n</script>");
  webpage += F("<script>\nfunction deleteFile(obj){\n $('input[name =\"delete\"]').val(obj.parentElement.parentElement.firstElementChild.childNodes[1].textContent); \n $('#form-delete').submit(); \n }\n</script>");
}
